...
extension Hourly{
    public static func forecasts( lat: Double, long: Double, temperature: String ,_ completion: @escaping ([Hourly]) -> Void ) {
        var scale: String = "si"
        if temperature.isEmpty{
            scale = "si"
        }else if temperature == "us"{
            scale = "us"
        }else if temperature == "is"{
            scale = "si"
        }
        let urlString = "https://api.darksky.net/forecast/daef8b9872252b375ea2a954c2a66c17/\(lat),\(long)?units=" + scale
        print(urlString)
